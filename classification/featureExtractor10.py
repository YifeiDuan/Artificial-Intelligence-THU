...
class KMeansClusterDigit(BaseFeatureExtractor):
    ...
    def fit(self, trainingData):
        ...
        "*** YOUR CODE HERE ***"
        while(1):
            lastClusters = self.clusters.copy()
            dictOfClusters = {}
            for key in range(self.num_cluster):
                dictOfClusters[key] = 0
            for key in cluster_no:
                dictOfClusters[key] += 1  # count how many elements there are in each cluster
            for i in range(n):
                self.clusters[cluster_no[i]] += trainingData[i] # add up the arrays in the same cluster
            for j in range(self.num_cluster):
                self.clusters[j] /= max(1,dictOfClusters[j]) # calculate centroids, the mean value of elements in the same cluster

            for m in range(n):
                min = float('inf') # initialize minimum distance as infinity for each trainingData
                newClusterLabel = cluster_no[m]
                for t in range(self.num_cluster):
                    temp = np.subtract(self.clusters[t], trainingData[m]) # substract current trainingData array from each line of self.clusters
                    distance = np.dot(temp, temp.T) # calculate the distance between current trainingData and each centroid
                    if(distance < min): newClusterLabel = t
                cluster_no[m] = newClusterLabel

            if (lastClusters == self.clusters).all(): break
            else: continue
