    ...
    def fit(self, trainingData):
        ...
        for iteration in range(self.num_iterations):
            ...
            for i in range(self.num_cluster):
                if i in cluster_no:
                    self.clusters[i] = np.mean(trainingData[np.where(cluster_no == i)], axis = 0)
            ...
            for m in range(n):
                ...
                for t in range(self.num_cluster):
                    ...
                    distance = np.dot(temp, temp) # calculate the distance between current trainingData and each centroid
                    if(distance < min): 
                        newClusterLabel = t
                        min = distance
