...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        "*** YOUR CODE HERE ***"
        ghostDistances = [manhattanDistance(newPos, newGhostStates[index].getPosition()) for index in range(len(newGhostStates))]
        nearestGhost = min(ghostDistances)
        if nearestGhost < 5: ghostScore = -(5 - nearestGhost)*5
        else: ghostScore = 0

        if len(newFood.asList()) > 0: foodDistances = [manhattanDistance(newPos, food) for food in newFood.asList()]
        else: foodDistances = [1]
        nearestFood = min(foodDistances)
        foodScore = 10/nearestFood
        
        totalScore = successorGameState.getScore() + ghostScore + foodScore
        return totalScore
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        alpha = -float('inf')
        beta = float('inf')
        minimaxResult = self.AlphaBeta(gameState,currentAgent, currentDepth, alpha, beta)
        return minimaxResult[0]
